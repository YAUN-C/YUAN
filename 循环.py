...
def insert():
    ...
    while True:
        ...
        try:
            ...
            print("输入无效，请重新输入")
        # 将录入的学生信息保存在字典中
        student = {'id': id, 'name': name, 'banji':banji, '英语': english, '语文': chinese, '数学':math }
        # 将学生信息添加到列表中
...
def show_query(lst):
    if len(lst) == 0:
        ...
        return
    # 定义标题显示格式
    format_title = '{:^6}\t{:^12}\t{:^8}\t{:^10}\t{:^10}\t{:^10}\t{:^9}'
    print(format_title.format('ID', '姓名','班级', '英语成绩', '语文成绩', '数学成绩', '总成绩'))
    # 定义内容显示格式
    format_data = '{:^6}\t{:^12}\t{:^8}\t{:^10}\t{:^10}\t{:^10}\t{:^9}'
    for item in lst:
...
def sort():
    ...
    if asc_or_desc == '0':
        ...
        print("输入有误，请重新输入")
        sort()
    mode = input("请选择排序方式(1.英语 2.语文 3.数学 4.总成绩)：")
    if mode == '1':
        ...
        students_new.sort(key=lambda x: int(x['english']) + int(x['chinese']) + int(x['math']), reverse=asc_or_desc_bool)
    else:
        print("输入有误，请重新输入")
